...
for split, ds in image_datasets.items():
    print(split, "size:", len(ds), "classes:", ds.classes)

dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=4,
                                             shuffle=True, num_workers=0)  # Windows 建議 num_workers=0
              for x in ['train', 'val']}
dataset_sizes = {x: len(image_datasets[x]) for x in ['train', 'val']}
class_names = image_datasets['train'].classes


# 補充：本地建議直接用 torch.device
...
# 測試圖片請放一張在 cnn 資料夾下，例如 test.jpg
...
if os.path.exists(test_img_path):
    ...
    print(f"測試圖片 {test_img_path} 不存在，請放一張圖片於 cnn 資料夾並命名為 test.jpg")
